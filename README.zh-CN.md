...
| 套件                                                                                            |     ★ | 技能数 | 最适合                                                                                                                                                | 运行            |
| --------------------------------------------------------------------------------------------- | ----: | --: | -------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| [academic-research-skills](https://github.com/Imbad0202/academic-research-skills)             | 39.8k |   — | 一条专为对抗 AI 编造而设计、刻意拒绝代笔的人在环路流水线；非商业许可证                                                                                                              | `net` `hooks` |
| [academic-research-skills-codex](https://github.com/imbad0202/academic-research-skills-codex) |  7.2k |   — | Codex 原生的学术研究 Skill 集：深度调研、论文起草、稿件评审、从研究到成文的流水线、引用与学术诚信核查。端到端覆盖面广；但以一整个巨型 skill 的形式打包内置（审查时需相应留意）；采用非商用 license                                    | `net` `hooks` |
...
| [aut_sci_write](https://github.com/shzhao27208/aut_sci_write)                                 |   172 |   — | 一个覆盖研究全生命周期的模块化 skill 集：跨 arXiv/PubMed/Web of Science 文献检索、PDF 与图表提取、综述/meta 分析写作、Zotero 同步以及幻灯片制作。覆盖面广、采用率高；属于“样样都能做”的大而全 skill 集，而非某一环节做到最好的专精工具 | `net`         |
...
| [claude-research](https://github.com/flonat/claude-research)                                  |   121 |   — | 面向博士研究者的基础设施：50 个 skill，外加 agent、hook 与规则，用于参考文献校验、LaTeX 健康检查、实验与因果设计，以及可复现的项目搭建。端到端能力很强；附带 hook 和一个未限定范围的 Bash                                    | `net` `hooks` |
...
| [nature-skills](https://github.com/Yuan1z0825/nature-skills)                                  | 31.7k |   9 | 一个精炼的九 skill 套件，最强在 Nature 风格的行文与配图，最弱在作为数据工具集                                                                                                     | `net`         |
...
| [papercash](https://github.com/jesseovo/papercash)                                            |    93 |   — | 面向中国学生、覆盖 8 个免费来源（S2、arXiv、Crossref、PubMed、CNKI、万方、百度/Google Scholar）的端到端论文工作流：检索、综述、润色、查重预检、降 AI 率、GB/T 7714 参考文献。覆盖广，但每个环节都偏浅                    | `net` `hooks` |
| [posit-dev/skills](https://github.com/posit-dev/skills)                                       |   443 |   — | 出自 RStudio 团队的 Posit 官方 R / Quarto / tidyverse skill——当分析建立在 R 而非 Python 上时的首选                                                                     | —             |
...
| [xueshuzhi-skills](https://github.com/yipng05-max/-skills)                                    |   252 |   — | 覆盖面很广的中文学术套件：选题、研究设计、CNKI/外文文献检索与核验、主题编码、综述与学位论文撰写，外加一条 12 检查点的 TA 流水线来拼装 Word 初稿。重广度而非深度                                                          | `net`         |
...
**claude-scientific-skills**, **claude-skills-journalism**, **deeppapernote**, **latex-document-skill**, **Awesome AI for Science**。（链接与描述见上方各生命周期分区。）
